...
export default async function ProfilePage() {
  ...
  const totalRaised = campaigns?.reduce((sum, campaign) => sum + parseFloat(campaign.current_amount || '0'), 0) || 0
  const totalGoal = campaigns?.reduce((sum, campaign) => sum + parseFloat(campaign.goal_amount || '0'), 0) || 0
          ...
          <div className="mb-8 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
            <div>
              ...
              <p className="mt-2 max-w-2xl text-sm leading-6 text-muted-foreground">
                Manage your account identity and review the activity tied to your campaigns.
              </p>
            </div>
              ...
              <Card className="border-border/70 shadow-sm">
                ...
                <CardContent className="space-y-3 text-sm">
                  ...
                  <p className="text-xs leading-5 text-muted-foreground">
                    Profile data is sourced from the `profiles` table. If you want editable fields here,
                    I can wire this page to a profile update form next.
                  </p>
                </CardContent>
              </Card>
            </div>
                    ...
                    <div className="space-y-2.5">
                      {campaigns.slice(0, 5).map((campaign) => {
                        const progress = Math.min(
                          (parseFloat(campaign.current_amount || '0') / parseFloat(campaign.goal_amount || '1')) * 100,
                          100
                        )
                            ...
                            <div className="flex items-start justify-between gap-3">
                              <div className="min-w-0">
                                ...
                                <p className="text-xs text-muted-foreground">
                                  MAD {parseFloat(campaign.current_amount || '0').toLocaleString()} raised
                                </p>
                              </div>
